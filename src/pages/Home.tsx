...
const Home = () => {
  ...
  function aumentarMesAtual() {
    const novaData = new Date(
      stateGeral.geral.dataAtualDoFiltro.getFullYear(),
      stateGeral.geral.dataAtualDoFiltro.getMonth() + 1,
      stateGeral.geral.dataAtualDoFiltro.getDay()
    );
  ...
  function filtroPelaDataAtual() {
    ...
    novaDespesasExibidas = novaDespesasExibidas.filter((despesaAtual) => {
      ...
      return (
        dataDaDespesaAtual.getMonth() === dataAtualDoFiltro.getMonth() &&
      ...
      <main>
        <section className={style.home_section}>
          <article className={style.home_article}>
            ...
            <input
              data-testid="home-page-input-dataDaDespesa"
              type="date"
              className={style.home_input}
          ...
          <article className={style.home_article}>
            <input
              data-testid="home-page-input-descricao"
              type={"text"}
              className={style.home_input_descricao}
              placeholder={"Descrição"}
              value={stateGeral.geral.descricao}
              onChange={(e) => dispatch(editaDescricao(e.target.value))}
            />
            <input
              data-testid="home-page-button-submit"
              type={"submit"}
              className={style.home_input}
            ...
            <table className={style.home_table}>
              <thead>
                <tr>
                  ...
                  <th className={style.home_table_td_th}>
                    Metodo de Pagamento
                  </th>
              ...
              <tbody
                className={style.home_table_body}
                data-testid="home-page-tbody"
              >
                {stateGeral.geral.despesas.length > 0 &&
                  stateGeral.geral.despesasExibidas.map(
                    (item: despesa, index: number) => {
                      let data = new Date(
        ...
        <section className={style.home_end_page}>
          <article className={style.home_article_edicao_data}>
            <button className={style.home_button} onClick={diminuirMesAtual}>
              mês anterior
            </button>

            <h1 className={style.home_texto_data}>
              {stateGeral.geral.dataAtualDoFiltro.getMonth() + 1} /
              {stateGeral.geral.dataAtualDoFiltro.getFullYear()}
            </h1>
            <button onClick={aumentarMesAtual} className={style.home_button}>
              próximo mês
            </button>
            <button
              onClick={handleChangeAplicarFiltro}
